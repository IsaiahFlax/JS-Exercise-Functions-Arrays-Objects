...
*/
function getName(objectContainingName) {
  return("Hello, my name is " +objectContainingName.name);
...
function getCarInfoByIndex(inventory, index) {
  var getCar = inventory.find((item, index) => {
    return index -1;
  })
...
function getLastCarInfo(inventory) {
  const theLast = inventory.find((item, index) => {
    return index === 49 // we use 2 because index is zero-based.
  })
...
function getCarInfoById(inventory, id) {
  var getCar = inventory.find((item, id) => {
    return id -1;
  })
  return `This is a ${getCar.car_make} ${getCar.car_model}`
}
...
function getModelYears(inventory) {
 let arrayContainingAllCarYears = [];
 ...
 * }
*/
const sum = (num1, num2) => { return num1 + num2; }; // code here!
...
function carMaker(number) {
  let updatedValue = {
   ...
   drive: function (distance) {
     updatedValue.odometer = number += distance;
     return updatedValue.odometer;
   }
 }
     return updatedValue;
}
